...
export class MaintenanceEventsPageService {
	...
	updateForm(idEvent: number): Observable<IResponse<IResposeEventById>> {
		return this._eventApiService.getEventById(idEvent).pipe(
			tap((response) => {// con tap se puede cambiar la inforamcion que viene del response
				if (response.success) {
	...
	saveEvent(method: CRUD_METHOD): Observable<boolean> {
		...
		const request: IRequestCreateEvent = {
			...
			dateEvent: this._datePipe.transform(this.dateField.value as Date, 'yyyy-MM-dd')!,
			timeEvent: this.hourField.value as string,
		...
		return this._confirmBoxEvokeService
			...
			.pipe(
				concatMap((responseQuestion) => (responseQuestion.success ? this._getMethod(method, request) : EMPTY)),//EL EMPTY MATA EL OBSERVABLE
				concatMap((response) => {
					if (response.success) {
